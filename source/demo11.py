...
def encode_decode_pipeline(original_audio, frame_size=160, verbose=False):
    ...
    bitstream0, prev_frame_st_resd = RPE_frame_coder(first_frame, np.zeros(frame_size))

    # Decode the first frame to get the reconstructed signal
    reconstructed_first_frame, _ = RPE_frame_decoder(bitstream0, np.zeros(frame_size))
    ...
    for i in range(1, n_frames):
        ...
        bitstream, curr_frame_st_resd = RPE_frame_coder(frame, prev_frame_st_resd)

        # Decode the frame using the encoded bitstream
        reconstructed_frame, _ = RPE_frame_decoder(bitstream, prev_frame_st_resd)
